...
class Swift(Pump, Keys, Gripper, Grove):
    ...
    def send_cmd_async(self, msg=None, timeout=None, callback=None, debug=True, enable_callback_thread=True):
        ...
        with self._cnt_lock:
            ...
            cmd = self.Cmd(self, self._cnt, msg, timeout, callback, debug=debug,
                           enable_callback_thread=enable_callback_thread)
    ...
    def get_is_moving(self, wait=True, timeout=None, callback=None, debug=True):
        ...
        if wait:
            ...
            self.send_cmd_async(cmd, timeout=timeout,
                                callback=functools.partial(_handle, _key='is_moving', _callback=callback), debug=debug)
    ...
    def set_3d_feeding(self, distance=0, speed=None, relative=True, x=None, y=None, z=None, wait=True, timeout=30,
        ...
        if self._current_temperature < 170:
            logger.error(
                'The Temperature must over 170 °C, current temperature is {}'.format(self._current_temperature))
            return
